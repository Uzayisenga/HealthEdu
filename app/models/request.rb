class Request < ApplicationRecord
  belongs_to :course
  belongs_to :user
  #@request.notify :users, key: "request.reply"
  ...
  # Set to notify to author and users commented to the course, except request owner self
  targets: ->(request, key) {
    ([request.course.user] + request.course.requested_users.to_a - [request.user]).uniq
  },
